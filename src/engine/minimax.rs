...
pub fn minimax<E: Evaluator>(
    ...
    ctx.stats.nodes_per_depth[ctx.stats.depth] += 1;
    ctx.stats.total_node_depth += ctx.stats.depth;
    ctx.stats.nodes += 1;
    ...
    if null_move_allowed && depth >= 3 && board.check.is_none() && has_non_pawn_material(board, active_player) {
        ...
        if active_player == Color::White {
            ...
            let null_score = minimax(board, depth - 3, opponent, eval, alpha, alpha + 1, ctx, false);
            board.en_passant = prev_ep;
            board.hash = prev_hash;
            if null_score <= alpha {
                return alpha;
    ...
    if active_player == Color::White {
        moves.sort_unstable_by_key(|mv| {
            std::cmp::Reverse(move_order_score(
                ...
                killer1, killer2, &ctx.history,
            ))
        });
        let mut max_eval = i32::MIN;
        for (i, &m) in moves.iter().enumerate() {
            // Futility Pruning
            if depth == 1 && m.capture == Free && !matches!(m.move_type, Promotion(_))
                && m.check.is_none() && board.score + QUEEN_VALUE < alpha
            {
                continue;
            }
            ...
            let score = if i == 0 {
                ...
                let is_quiet = m.capture == Free && !matches!(m.move_type, Promotion(_))
                    && m.check.is_none() && killer1 != Some(m) && killer2 != Some(m);
                let r: u8 = if is_quiet && depth >= 3 && i >= 3 { if i >= 6 { 2 } else { 1 } } else { 0 };
                let scout = minimax(board, (depth - 1).saturating_sub(r), opponent, eval, alpha, alpha + 1, ctx, true);
        ...
        if should_store {
            ctx.tt.insert(board.hash, TtEntry { score: max_eval, depth, flag });
        ...
        for (i, &m) in moves.iter().enumerate() {
            // Futility Pruning
            if depth == 1 && m.capture == Free && !matches!(m.move_type, Promotion(_))
                && m.check.is_none() && board.score - QUEEN_VALUE > beta
            {
                continue;
            }
            board.apply_move(&m, active_player);
            ...
            let score = if i == 0 {
                ...
                let is_quiet = m.capture == Free && !matches!(m.move_type, Promotion(_))
                    && m.check.is_none() && killer1 != Some(m) && killer2 != Some(m);
                let r: u8 = if is_quiet && depth >= 3 && i >= 3 { if i >= 6 { 2 } else { 1 } } else { 0 };
                let scout = minimax(board, (depth - 1).saturating_sub(r), opponent, eval, beta - 1, beta, ctx, true);
        ...
        let should_store = ctx.tt.get(&board.hash).map_or(true, |e| depth >= e.depth);
        if should_store {
            ctx.tt.insert(board.hash, TtEntry { score: min_eval, depth, flag });
            ctx.stats.tt_stores += 1;
        }
        min_eval
    }
    ...
    if active_player == Color::White {
        ...
        for (i, &m) in moves.iter().enumerate() {
            ...
            let score = if i == 0 {
                ...
                let scout = minimax(board, depth - 1, opponent, eval, alpha, alpha + 1, ctx, true);
                if scout > alpha {
                    minimax(board, depth - 1, opponent, eval, alpha, i32::MAX, ctx, true)
                } else {
                    scout
...
pub fn quiescence_minimax<E: Evaluator>(
    ...
    if depth <= 0 {
        return if active_player == Color::White { alpha } else { beta };
    }

    let mut move_list = MoveList::new();
    generate_moves(board, &active_player, &mut move_list, true);
    let opponent = if active_player == Color::White { Color::Black } else { Color::White };
    ...
    for i in 0..move_list.count {
        ...
        const DELTA: i32 = 200;
        if active_player == Color::White && stand_pat + capture_value + DELTA < alpha { continue; }
        if active_player == Color::Black && stand_pat - capture_value - DELTA > beta { continue; }
    ...
    if active_player == Color::White { alpha } else { beta }
}
