...
impl ChessApp {
    ...
    pub fn mobile_turn_infos(&mut self, ui: &mut egui::Ui) {
        ui.label(RichText::new(format!("Turn #{}", self.game.turn)).size(40.0));
        if let Some(end) = &self.game.end {
            match end {
                End::Checkmate => ui.label(
                    RichText::new(format!("Checkmate ! {:?} win", self.game.opponent()))
                        .size(40.0),
                ),
            ...
            if self.game.board.check.is_some() {
                ui.label(
                    RichText::new(format!("Check ! {:?} to move", self.game.active_player))
                        .size(40.0),
                );
            } else {
                ui.label(
                    RichText::new(format!("{:?} to move", self.game.active_player)).size(40.0),
                );
            }
        }
    }
