...
impl ChessApp {
    ...
    pub fn mobile_turn_infos(&mut self, ui: &mut egui::Ui) {
        ...
        if let Some(end) = &self.current.end {
            match end {
                End::Checkmate => ui.label(RichText::new(format!("Checkmate ! {:?} win", self.current.opponent)).size(40.0)),
                End::TimeOut => ui.label(RichText::new(format!(
                    "{:?} out of time !\n{:?} win",
                ...
                End::Resign => ui.label(RichText::new(format!(
                    "{:?} resigned : {:?} win",
                    self.current.active_player, self.current.opponent
                )).size(40.0)),
            };
