...
fn pawn_threats(cell: &Cell, coord: &Coord, color: &Color, threaten_cells: &mut Vec<Coord>) {
    let offsets: [(i8, i8); 2] = if cell.is_color(&White) {
        [(1, -1), (1, 1)]
    } else {
        [(-1, -1), (-1, 1)]
    };
...
fn rook_threats(
    ...
    if row < 7 {
        threaten_in_line(coord, row as u8 + 1, col as u8, board, threaten_cells);
    }
    if row > 0 {
        threaten_in_line(coord, row as u8 - 1, col as u8, board, threaten_cells);
    }
    if col < 7 {
        threaten_in_line(coord, row as u8, col as u8 + 1, board, threaten_cells);
    }
    if col > 0 {
        threaten_in_line(coord, row as u8, col as u8 - 1, board, threaten_cells);
    }
...
fn bishop_threats(
    ...
    if row < 7 && col < 7 {
        threaten_in_diag(coord, row as u8 + 1, col as u8 + 1, board, threaten_cells);
    }
    if row < 7 && col > 0 {
        threaten_in_diag(coord, row as u8 + 1, col as u8 - 1, board, threaten_cells);
    }
...
fn threaten_in_line(
    ...
    match (row.cmp(&from.row), col.cmp(&from.col)) {
        (std::cmp::Ordering::Greater, _) if row < 7 => {
            threaten_in_line(from, row + 1, col, board, threaten_cells)
        }
        (std::cmp::Ordering::Less, _) if row > 0 => {
            threaten_in_line(from, row - 1, col, board, threaten_cells)
        }
        (_, std::cmp::Ordering::Greater) if col < 7 => {
            threaten_in_line(from, row, col + 1, board, threaten_cells)
        }
        (_, std::cmp::Ordering::Less) if col > 0 => {
            threaten_in_line(from, row, col - 1, board, threaten_cells)
        }
        _ => {}
    }
