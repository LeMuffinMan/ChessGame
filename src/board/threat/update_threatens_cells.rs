...
impl Board {
    pub fn update_threatens_cells(&mut self) {
        ...
        for row in 0..8 {
            for col in 0..8 {
                ...
                {
                    match piece {
                        Pawn => self.pawn_threats(&cell, &coord),
                        Rook => self.rook_threats(&coord, row, col),
                        Knight => self.knight_threats(&coord, &cell),
                        Bishop => self.bishop_threats(&coord, row, col),
                        Queen => {
                            self.rook_threats(&coord, row, col);
                            self.bishop_threats(&coord, row, col);
                        }
                        King => self.king_threats(&coord),
                    }
                } else {
    ...
    fn pawn_threats(&mut self, cell: &Cell, coord: &Coord) {
        ...
        for (dr, dc) in cells {
            ...
            if (0..8).contains(&new_row) && (0..8).contains(&new_col) {
                ...
                if !cell.is_color(&self.active_player) {
                    self.threaten_cells.push(Coord {
                        row: new_row as u8,
                        col: new_col as u8,
                    });
                }
    ...
    fn rook_threats(&mut self, coord: &Coord, row: usize, col: usize) {
        if row < 7 {
            get_threaten_cells_in_line(coord, row as u8 + 1, col as u8, self);
        }
        if row > 0 {
            get_threaten_cells_in_line(coord, row as u8 - 1, col as u8, self);
        }
        if col < 7 {
            get_threaten_cells_in_line(coord, row as u8, col as u8 + 1, self);
        }
        if col > 0 {
            get_threaten_cells_in_line(coord, row as u8, col as u8 - 1, self);
        }
    ...
    fn knight_threats(&mut self, coord: &Coord, cell: &Cell) {
        let cells: [(i8, i8); 8] = [
            (2, 1),
            (2, -1),
            (-2, 1),
    ...
    fn bishop_threats(&mut self, coord: &Coord, row: usize, col: usize) {
        if row < 7 && col < 7 {
            get_threaten_cells_in_diag(coord, row as u8 + 1, col as u8 + 1, self);
        }
        if row < 7 && col > 0 {
            get_threaten_cells_in_diag(coord, row as u8 + 1, col as u8 - 1, self);
        }
    ...
    fn king_threats(&mut self, coord: &Coord) {
        let cells: [(i8, i8); 8] = [
            (-1, -1),
            (-1, 0),
            (-1, 1),
