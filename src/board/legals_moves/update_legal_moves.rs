...
impl Board {
    ...
    pub fn update_legals_moves(&mut self) {
        ...
        for x in 0..8 {
            for y in 0..8 {
                if self.grid[x][y].is_color(&self.active_player) {
                    let from = Coord {
                        row: x as u8,
                        col: y as u8,
                    };
                    if let Some(piece) = self.get(&from).get_piece() {
                        match piece {
                            Pawn => {
                                let vec = self.update_pawn_legals_moves(&from);
                                self.legals_moves.extend(vec);
                            }
                            Rook => {
                                let vec = self.update_rook_legals_moves(&from);
                                self.legals_moves.extend(vec);
                            }
                            Knight => {
                                let vec = self.update_knight_legals_moves(&from);
                                self.legals_moves.extend(vec);
                            }
                            Bishop => {
                                let vec = self.update_bishop_legals_moves(&from);
                                self.legals_moves.extend(vec);
                            }
                            Queen => {
                                let vec = self.update_queen_legals_moves(&from);
                                self.legals_moves.extend(vec);
                            }
                            King => {
                                let vec = self.update_king_legals_moves(&from);
                                self.legals_moves.extend(vec);
                            }
                        }
                    }
    ...
    pub fn test_and_push(
        ...
        if let Ok(_) = self.is_legal_move(from, to) && let Ok(_) = self.is_king_exposed(from, to) {
            return Some((*from, *to));
        }
        None
    }
